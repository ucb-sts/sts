(function() {
  ...
  d3.timeline = function() {
    ...
    var hover = function () {},
        ...
        beginning = 0,
        ending = 0,
        margin = {left: 30, right:400, top: 50, bottom:1000},
        stacked = false,
        rotateTicks = false,
        itemHeight = 20,
        itemMargin = 5
    ...
    function timeline (gParent) {
      ...
      g.each(function(d, i) {
        d.forEach( function(datum, index){
          ...
          // keeping track of event SR_id 
          for (i = 0; i< data.length; i++){
            if ( ! (data[i].SR_id in eventsSendReceived)){ 
                eventsSendReceived[data[i].SR_id] = [getXPos(data[i],i), getStackPosition(data[i],i)];
              } else {
                eventsSendReceived[data[i].SR_id].push(getXPos(data[i],i));
                eventsSendReceived[data[i].SR_id].push(getStackPosition(data[i],i));
            }
          }
      ...
      // draw lines appropriately between send/receive messages between controllers and switches
      for (var e in eventsSendReceived){
        if (eventsSendReceived[e].length == 4){
          g.append('line')
            .attr("x1", eventsSendReceived[e][0])
            .attr("y1", eventsSendReceived[e][1])
            .attr("x2", eventsSendReceived[e][2])
            .attr("y2", eventsSendReceived[e][3])
            .style("stroke","rgb(0,0,0)");          
        }
      }
