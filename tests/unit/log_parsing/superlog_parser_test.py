...
class superlog_parser_test(unittest.TestCase):
  ...
  def open_simple_superlog(self):
    ...
    e1 = str('''{"dependent_labels": ["e2"], "start_dpid": 1, "class": "LinkFailure",'''
             ''' "start_port_no": 1, "end_dpid": 2, "end_port_no": 1, "label": "e1", "event_time": [0,0], "logical_round": 0}''')
    superlog.write(e1 + '\n')
    e2 = str('''{"dependent_labels": [], "start_dpid": 1, "class": "LinkRecovery",'''
             ''' "start_port_no": 1, "end_dpid": 2, "end_port_no": 1, "label": "e2", "event_time": [0,0], "logical_round": 0}''')
    superlog.write(e2 + '\n')
    superlog.close()
