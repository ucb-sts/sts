...
class UserSpaceControllerPatchPanelTest(unittest.TestCase):
  def _setup_patch_panel(self, block_controllers=False):
    # Have two net_ns processes ping eachother, and verify that the pings go through.
    p = UserSpaceControllerPatchPanel()
    ...
    p.register_controller("c1", ping1_eth_addr, ping1_pcap)
    ...
    p.register_controller("c2", ping2_eth_addr, ping2_pcap)
    return p
